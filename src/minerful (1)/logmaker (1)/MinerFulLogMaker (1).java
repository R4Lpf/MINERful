...
public class MinerFulLogMaker {
	...
	public XLog createLog(ProcessModel processModel) {
		...
		/**
		File nCFile = this.parameters.negativeConstraintsFile;
		 File n = new File("/home/ralph/Desktop/minerful3/constraints.txt");
		String ncf = "";
		try{
		...
		/**
		String[] ncobj = Arrays.stream(this.parameters.negativeConstraints.split(",")).map(String::trim).toArray(String[]::new);
		List<String> ncList = Arrays.asList(ncobj);
		 Set<String> set = new HashSet<String>(ncList);
		 **/
